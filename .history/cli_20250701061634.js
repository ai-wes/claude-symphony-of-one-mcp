...
class OrchestratorCLI {
 ...
 |____/ |_| |_|  |_|_|   |_| |_|\\___/|_| \\_| |_|    \\___/|_|      \\___/|_| \\_|_____|
  ...
  async createCustomRole() {
    console.log(chalk.yellow("🎭 Create Custom Role"));
    ...
    const roleData = await inquirer.prompt([
      ...
      },
      {
        type: "editor",
        name: "prompt",
        message: "Role prompt/instructions (opens editor):",
    ...
    // Store custom role (in a real implementation, this would be persisted)
    const roleKey = `CUSTOM_${roleData.name
      .toUpperCase()
      .replace(/\s+/g, "_")}`;
    console.log(
      chalk.green(
        `✓ Custom role "${roleData.name}" created with key: ${roleKey}`
      )
    );
    console.log(
      chalk.gray("Note: Custom roles are not persisted in this version")
    );
  }

  async removeAgentRole(agentId) {
    if (!agentId) {
    ...
    if (this.agentRoles.has(agentId)) {
      ...
      console.log(chalk.green(`✓ Removed role "${role.name}" from ${agentId}`));

      // Notify the agent
      await this.tagSpecificAgent(
        agentId,
        `🎭 **Role Removed**\n\nYour role assignment as "${role.name}" has been removed. You now have no specific role assigned.`
      );
    } else {
      console.log(chalk.yellow(`No role assigned to ${agentId}`));
    }
  }

  async sendRolePrompt(agentId) {
    if (!agentId) {
      console.log(chalk.red("Please specify an agent ID"));
      return;
    }

    const roleData = this.agentRoles.get(agentId);
    if (!roleData) {
      console.log(chalk.yellow(`No role assigned to ${agentId}`));
      return;
    }

    const message = `🎭 **Role Reminder: ${roleData.name}**\n\n${
      roleData.prompt
    }\n\n**Your Capabilities:** ${roleData.capabilities.join(
      ", "
    )}\n\nPlease confirm you understand your role and are ready to take on tasks matching these responsibilities.`;

    await this.tagSpecificAgent(agentId, message);
    console.log(chalk.green(`✓ Sent role prompt to ${agentId}`));
  }
  ...
  async createCustomTemplate() {
    console.log(chalk.yellow("📋 Create Custom Task Template"));

    const templateData = await inquirer.prompt([
      {
        type: "input",
        name: "title",
        message: "Template title (use {variable} for placeholders):",
      ...
      },
      {
        ...
        name: "description",
        message:
          "Template description (opens editor, use {variable} for placeholders):",
        validate: (input) =>
          input.trim() !== "" || "Description cannot be empty",
      },
      {
      ...
      },
      {
        type: "input",
        name: "assignedRole",
        message: "Assigned role (leave empty for any role):",
      },
      {
        type: "input",
        name: "estimatedHours",
    ...
    const customTemplate = {
      ...
      assignedRole: templateData.assignedRole || null,
    ...
    const templateKey = `CUSTOM_${templateData.title
      .toUpperCase()
      .replace(/[^A-Z0-9]/g, "_")}`;

    console.log(
      chalk.green(`✓ Custom template created with key: ${templateKey}`)
    );
    console.log(chalk.cyan("Template preview:"));
    console.log(chalk.gray(`  Title: ${customTemplate.title}`));
    console.log(chalk.gray(`  Priority: ${customTemplate.priority}`));
    console.log(chalk.gray(`  Est. Hours: ${customTemplate.estimatedHours}`));
    console.log(
      chalk.gray(`  Checklist: ${customTemplate.checklist.length} items`)
    );
    console.log(
      chalk.gray("Note: Custom templates are not persisted in this version")
    );
  }
